...
impl Armed {
    ...
    async fn run(self) {
        ...
        // The crown-mode indicator. A no-op on the platforms without one, so
        // the mode channel is drained everywhere and cannot fill.
        let crown_osd = crate::osd::spawn();
        ...
        loop {
            tokio::select! {
                ...
                Some(indicator) = running.inputs.mode_changes.recv() => {
                    crown_osd.show(&indicator);
                }
