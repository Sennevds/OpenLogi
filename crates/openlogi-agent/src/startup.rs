...
use openlogi_agent_core::runtime::{ActionDispatcher, ActionRuntime, ActionRuntimeParams};
...
pub(crate) struct InputServices {
    ...
    /// Crown-mode changes awaiting presentation. Consumed on every platform so
    /// the channel cannot fill; only Windows currently draws them.
    pub(crate) mode_changes:
        tokio::sync::mpsc::UnboundedReceiver<openlogi_agent_core::CrownModeIndicator>,
...
impl InputServices {
    fn start(shared: &SharedRuntime) -> Option<Self> {
        ...
        let action_runtime = match ActionRuntime::new(ActionRuntimeParams {
            dpi_cycle: shared.dpi_cycle.clone(),
            crown_modes: shared.crown_modes.clone(),
            capture: shared.capture_channel.clone(),
            registry: shared.channel_registry.clone(),
            receiver_access: shared.receiver_access.clone(),
            device_io: shared.device_io.clone(),
            action_ring: sender,
            mode_indicator: mode_sender,
        }) {
            Ok(runtime) => runtime,
            Err(e) => {
