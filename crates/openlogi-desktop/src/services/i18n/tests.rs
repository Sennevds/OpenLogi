...
fn typed_translation_keys_resolve() {
    ...
    // The crown's controls sit outside `ButtonId::ALL` (they are neither mouse
    // hotspots nor F-row keys) but the Crown panel renders their labels through
    // `t!`, so they need the same coverage.
