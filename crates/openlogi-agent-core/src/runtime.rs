...
/// Everything the action executor needs, named rather than positional.
///
/// Eight handles in a row — three of them `Arc`s and two of them senders — is
/// where a transposed argument hides in plain sight at the call site.
pub struct ActionRuntimeParams {
    /// Per-device DPI cycle positions.
    pub dpi_cycle: Arc<RwLock<DpiCycles>>,
    /// Per-device crown-mode positions.
    pub crown_modes: Arc<RwLock<CrownModes>>,
    /// The capture session's published channel, for hardware actions.
    pub capture: CaptureChannel,
    /// Live channel registry, for actions that write to a device.
    pub registry: ChannelRegistry,
    /// Receiver arbitration, so an action cannot race pairing.
    pub receiver_access: ReceiverAccess,
    /// The suspend gate: no device I/O while it forbids it.
    pub device_io: DeviceIoGate,
    /// Where `ShowActionsRing` invocations go.
    pub action_ring: tokio::sync::mpsc::UnboundedSender<Option<String>>,
    /// Where crown-mode changes go, for whatever presents them.
    pub mode_indicator: tokio::sync::mpsc::UnboundedSender<CrownModeIndicator>,
}

impl ActionRuntime {
    /// Build the action executor and its source-independent button worker.
    pub fn new(params: ActionRuntimeParams) -> io::Result<Self> {
        let ActionRuntimeParams {
            dpi_cycle,
            crown_modes,
            capture,
            registry,
